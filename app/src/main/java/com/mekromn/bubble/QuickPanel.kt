...
/** Bounded native popup for Activity/overlay hosts; no extra Activity or browser surface. */
internal class QuickPanel private constructor(val anchor: View, private val workspace: Workspace,
    ...
    val content = LinearLayout(anchor.context).apply {
        orientation = LinearLayout.VERTICAL; background = Ui.shape(context, Ui.SURFACE, 24f, Ui.LINE)
    ...
    init {
        ...
        content.addView(header); content.addView(body, LinearLayout.LayoutParams(-1, 0, 1f))
        ...
        popup.softInputMode = WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN or WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
        popup.enterTransition = Fade().apply { duration = 140 }; popup.exitTransition = Fade().apply { duration = 100 }
        ...
        val above = location[1] - height - dp(6); val below = location[1] + anchor.height + dp(6)
        ...
        try {
            ...
            // Report the error class, never title, URL, query, local notes or prompt contents.
            lastOpenFailure = failure.javaClass.simpleName
            if (BuildConfig.DEBUG) Log.w("BubbleQuickPanel", "Window attachment failed: $lastOpenFailure")
            runCatching { popup.dismiss() }; cleanup()
            Toast.makeText(anchor.context, "Could not open this quick menu. Your chat is unchanged.", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    fun finish(action: () -> Unit) {
        dismiss(); Handler(Looper.getMainLooper()).post { if (anchor.isAttachedToWindow) action() }
    }
    private fun cleanup() {
        if (cleaned) return
    ...
    companion object {
        ...
        internal var openAttempts = 0L
            private set
        internal var lastOpenFailure: String? = null
            private set
        /** Nested actions anchor to the persistent toolbar, not a dismissed popup's row. */
        fun hostAnchor(view: View): View = current.get()?.takeIf {
            it.showing && view.rootView === it.content.rootView
        }?.anchor ?: view
        fun open(anchor: View, workspace: Workspace, title: String, heightDp: Int): QuickPanel? {
            val host = hostAnchor(anchor)
            openAttempts++; lastOpenFailure = null
            if (!host.isAttachedToWindow || !workspace.ready) { lastOpenFailure = "HostNotReady"; return null }
            dismiss(); return QuickPanel(host, workspace, title, heightDp).takeIf { it.showing }
        }
