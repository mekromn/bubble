...
/** Reveal from the bubble, conceal to it. Cancelling never leaves an invisible/half-owned view. */
internal class WindowMotion {
    private var running: Animator? = null
    private var pendingView: View? = null
    private var beforeDraw: ViewTreeObserver.OnPreDrawListener? = null
    private var epoch = 0
    var busy = false
        private set
    fun cancel() {
        ...
        pendingView?.let { view ->
            beforeDraw?.let { if (view.viewTreeObserver.isAlive) view.viewTreeObserver.removeOnPreDrawListener(it) }
            view.alpha = 1f
        }
        beforeDraw = null; pendingView = null
        running?.removeAllListeners(); running?.cancel(); running = null; busy = false
    }
    fun reveal(view: View, x: Int, y: Int, bubbleRadius: Float, opening: Boolean, done: () -> Unit = {}) {
        ...
        val mine = epoch; busy = true; pendingView = view
        val start = {
            if (mine == epoch && view.isAttachedToWindow) {
                ...
                val animation = ViewAnimationUtils.createCircularReveal(view, cx, cy, if (opening) bubbleRadius else radius, if (opening) radius else bubbleRadius)
                animation.duration = if (opening) 260L else 210L; animation.interpolator = Ui.ease
                animation.addListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator) {
                        if (mine == epoch) { running = null; pendingView = null; busy = false; done() }
                    }
                })
                running = animation; animation.start()
            } else if (mine == epoch) { pendingView = null; busy = false }
        }
        if (opening) {
            view.alpha = 0f
            val listener = object : ViewTreeObserver.OnPreDrawListener {
                override fun onPreDraw(): Boolean {
                    ...
                    beforeDraw = null; start(); return true
                }
            }
            beforeDraw = listener
