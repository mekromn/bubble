...
/** Hysteresis prevents repeated haptics or flickering when the finger rests on the boundary. */
...
/** A small, NON-TOUCHABLE target shown only during a drag, never a fullscreen interception layer. */
internal class DismissTarget(private val context: Context, private val manager: WindowManager) {
    ...
    fun show(safe: WindowBox) {
        ...
        val params = WindowManager.LayoutParams(size, size, WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,
            ...
            x = (centerX - size / 2).toInt(); y = (centerY - size / 2).toInt()
            title = "Bubble hide target"
            alpha = .8f
        }
        RenderPolicy.vote(context, mark, params)
        try { manager.addView(mark, params); view = mark }
    ...
    fun track(x: Float, y: Float): Boolean {
        ...
        if (next != armed) {
            ...
            mark.animate().scaleX(if (next) 1.12f else 1f).scaleY(if (next) 1.12f else 1f)
                .setDuration(120).setInterpolator(Ui.ease).start()
        }
        return armed
    }
    fun hide(immediate: Boolean = false) {
        val mark = view ?: return
        epoch++; val mine = epoch
        armed = false
        mark.animate().cancel()
        if (immediate) { remove(mark); return }
        mark.animate().alpha(0f).translationY(d(16).toFloat()).setDuration(130)
            .withEndAction { if (epoch == mine) remove(mark) }.start()
    }
    ...
    private class Mark(context: Context) : View(context) {
        ...
        override fun onDraw(c: Canvas) {
            ...
            paint.style = Paint.Style.FILL; paint.color = if (armed) 0xffa84954.toInt() else Ui.SURFACE_HIGH
            c.drawCircle(54f, 46f, 29f, paint)
            paint.style = Paint.Style.STROKE; paint.strokeWidth = 1.5f; paint.color = if (armed) 0xffffced3.toInt() else Ui.MUTED
            c.drawCircle(54f, 46f, 28f, paint)
            paint.strokeWidth = 2.5f; paint.color = Ui.TEXT
            c.drawLine(46f, 38f, 62f, 54f, paint); c.drawLine(62f, 38f, 46f, 54f, paint)
            paint.style = Paint.Style.FILL; paint.textAlign = Paint.Align.CENTER; paint.textSize = 11f
            c.drawText(if (armed) "Release to hide" else "Hide in notification", 54f, 91f, paint)
            c.restore()
        }
    }
}
