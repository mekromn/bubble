...
/** Small, non-touchable black-glass target. Hiding is reversible, so capture uses a neutral
 * light ring rather than a destructive red state. Removed after each drag. */
internal class DismissTarget(private val context: Context,
    ...
    fun show(safe: WindowBox) {
        ...
        val size=d(108); centerX=safe.x+safe.width/2f; centerY=safe.y+safe.height-d(56).toFloat()
    ...
    private class Mark(context: Context): View(context) {
        ...
        private val glass=LinearGradient(25f,25f,80f,83f,0xe53e3e3e.toInt(),0xf5050505.toInt(),Shader.TileMode.CLAMP)
        init { importantForAccessibility=IMPORTANT_FOR_ACCESSIBILITY_NO }
        override fun onDraw(c: Canvas) {
            c.save(); c.scale(width/108f,height/108f)
            paint.style=Paint.Style.FILL
            if(armed) { paint.shader=null; paint.color=0xffd2d2d2.toInt() } else paint.shader=glass
            c.drawCircle(54f,54f,29f,paint)
            paint.shader=null; paint.style=Paint.Style.STROKE; paint.strokeWidth=1.2f
            paint.color=if(armed)Ui.TEXT else Ui.MUTED; c.drawCircle(54f,54f,28f,paint)
            paint.strokeWidth=2.5f; paint.color=if(armed)Ui.BG else Ui.TEXT
            c.drawLine(46f,46f,62f,62f,paint); c.drawLine(62f,46f,46f,62f,paint)
            paint.style=Paint.Style.FILL; paint.color=Ui.TEXT; paint.textAlign=Paint.Align.CENTER; paint.textSize=10f
            c.drawText(if(armed)"Release to hide" else "Hide in notification",54f,99f,paint); c.restore()
        }
    }
}
