            ...
            override fun onMessage(nativeApp: String, message: Any, sender: WebExtension.MessageSender): GeckoResult<Any>? {
                ...
                return when (payload.optString("event")) {
                    ...
                    "vault-transcript-begin" -> {
                        ...
                        if (ChatTranscriptCommandState.completed(context, tabId) == fingerprint ||
                            !activeTranscriptFingerprints.add(fingerprint)) {
                        ...
                        ChatTranscriptExports.begin(context, vault, sender.url, profileId) { export ->
                            if (export == null) {
                                ...
                                result.complete(JSONObject()
                                    .put("available", true)
                                    .put("transfer", export.transfer)
                                    .put("sourceId", export.sourceId)
                                    .put("filename", export.filename)
                                    .put("bytes", export.bytes)
                                    .put("chunkBytes", TRANSCRIPT_CHUNK_BYTES))
                            }
                        }
                        result
                    }
                    "vault-transcript-chunk" -> {
                        ...
                        val active = transcriptExports[transfer]
                            ?: return GeckoResult.fromValue(JSONObject().put("ok", false))
                        val result = GeckoResult<Any>()
                    ...
                    "vault-transcript-complete" -> {
                        val transfer = payload.optString("transfer")
                        val active = transcriptExports.remove(transfer)
                        if (active != null) {
