...
/** Actual page-start web signals and real touch input, not programmatic long-clicks. */
...
class LiveToolsRuntimeTest {
    ...
    private fun await(message: String,test: () -> Boolean) {
        ...
        screenshot("v070-failure.png")
        var state=""
        instrumentation.runOnMainSync {
            state="menuAttempts=${QuickPanel.openAttempts}\nlastFailure=${QuickPanel.lastOpenFailure}\n"+
                "popupVisible=${Workspace.peek()?.quickMenuVisible}\nmode=${BubbleService.active?.window?.mode}\n"
        }
        File(folder(),"v070-failure-state.txt").writeText(message+"\n"+state)
        fail(message)
    }
    private fun node(match: (AccessibilityNodeInfo) -> Boolean): AccessibilityNodeInfo? {
        ...
        automation.windows.sortedByDescending { it.layer }.forEach { walk(it.root)?.let { n -> return n } }
        return null
    }
    ...
    private fun touch(match: (AccessibilityNodeInfo) -> Boolean,hold: Boolean=false) {
        ...
        var attemptsBefore=0L
        if(hold)instrumentation.runOnMainSync { attemptsBefore=QuickPanel.openAttempts }
        ...
        if(hold) {
            // SwiftShader can deliver a frame >700ms late. Keep the real finger DOWN until
            // the long-click handler runs (bounded), not until a guessed wall-clock delay.
            // Never call performLongClick, alter a production timeout or bypass actual input.
            val deadline=SystemClock.elapsedRealtime()+4000
            var handled=false
            while(!handled && SystemClock.elapsedRealtime()<deadline) {
                Thread.sleep(100); handled=onMain { QuickPanel.openAttempts>attemptsBefore }
            }
