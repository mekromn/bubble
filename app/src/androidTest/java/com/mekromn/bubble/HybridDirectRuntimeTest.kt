...
class HybridDirectRuntimeTest {
    ...
    private fun inputHost():android.view.View?{var value:android.view.View?=null;main{value=Workspace.peek()?.selected?.session?.textInput?.view};return value}
    ...
    private fun pageIs(magenta:Boolean):Boolean{
        ...
        main{Workspace.peek()?.selected?.session?.textInput?.view?.let{v->
            val p=IntArray(2);v.getLocationOnScreen(p);x=p[0]+v.width/8;y=p[1]+v.height*4/5
    ...
    private fun tap(x:Float,y:Float){
        ...
        try{assertTrue(auto.injectInputEvent(d,true));assertTrue(auto.injectInputEvent(u,true))}finally{d.recycle();u.recycle()}
    }
    private fun pageTap(){
        var x=0f;var y=0f
        main{val v=requireNotNull(Workspace.peek()?.selected?.session?.textInput?.view);val p=IntArray(2);v.getLocationOnScreen(p);x=p[0]+v.width*.5f;y=p[1]+v.height*.46f}
        tap(x,y)
    }
        ...
        val serving=Thread{
            while(!server.isClosed)try{
                server.accept().use{s->
                    ...
                    val html="""<!doctype html><meta name=viewport content='width=device-width,initial-scale=1'><title>HYBRID-MAGENTA</title><style>html,body{margin:0;height:100%;overflow:hidden}canvas{position:fixed;inset:0;width:100%;height:100%}button{position:absolute;left:10%;top:40%;width:80%;height:12%;z-index:2;font-size:22px}</style><canvas id=c></canvas><button onclick='flip=!flip;paint();document.title=flip?"HYBRID-CYAN":"HYBRID-MAGENTA"'>toggle</button><script>const c=document.querySelector('canvas'),g=c.getContext('2d');let flip=false;function paint(){c.width=innerWidth;c.height=innerHeight;g.fillStyle=flip?'rgb(20,200,210)':'rgb(220,30,180)';g.fillRect(0,0,c.width,c.height)}paint()</script>""".toByteArray()
        ...
        try{
            ...
            ActivityScenario.launch<BrowserActivity>(Intent(context,BrowserActivity::class.java)).use{scenario->
                ...
                scenario.onActivity{a->
                    ...
                    a.workspace.create("http://127.0.0.1:${server.localPort}/hybrid")
                }
                ...
                // Do not let the transition snapshot/fullscreen source masquerade as a live page.
                await("fullscreen-source-hidden",30_000){fullscreenSourceGone()}
                await("direct-floating-live",90_000){directFloating()&&pageIs(true)&&relayIdle()}
                save("direct-floating-live")
                assertTrue("same GeckoSession in direct floating",checkMain{Workspace.peek()?.selected?.session===original})

                pageTap()
                await("direct-click",30_000){checkMain{Workspace.peek()?.selected?.title=="HYBRID-CYAN"}&&pageIs(false)}
                assertTrue("relay stays idle after direct DOM input",relayIdle())
                ...
                assertTrue("no Bubble relay in fullscreen",relayIdle())
                save("fullscreen-after-expand")

                main{requireNotNull(Workspace.peek()?.host?.get()).collapse(FloatingMode.CHAT)}
                await("fullscreen-source-hidden-again",30_000){fullscreenSourceGone()}
                ...
                pageTap()
                await("direct-second-click",30_000){checkMain{Workspace.peek()?.selected?.title=="HYBRID-MAGENTA"}&&pageIs(true)}
                assertTrue("relay stays idle after second direct DOM input",relayIdle())
                save("direct-after-shrink")

                evidence().resolve("hybrid-direct.txt").writeText(
                    "PASS: GeckoView's original SurfaceView remained the steady-state renderer; live pixels were verified only after the fullscreen source disappeared; DOM input worked in both floating epochs; relay stats stayed idle; same GeckoSession survived floating->fullscreen and fullscreen->floating snapshot morphs. ${FullscreenHandoff.debugSummary()}\n"
                )
            }
        }finally{
