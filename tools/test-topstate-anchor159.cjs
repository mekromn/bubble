...
assert.match(anchor,/ActivityManager\.getMyMemoryState/);
assert.ok(!/Handler|postDelayed|Choreographer|RenderPolicy|Gecko|setFrameRate|preferredRefreshRate/.test(anchor),
...
assert.ok(!/TYPE_APPLICATION_OVERLAY|WindowManager|updateViewLayout/.test(direct.replace(/\/\*[\s\S]*?\*\//g,'').replace(/\/\/.*$/gm,'')));
